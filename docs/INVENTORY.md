...
### 5-1. ⚠️ 최우선 — `BrowserRouter`가 `file://`에서 동작하지 않는다
...
### 5-4. `localStorage` 3개소 — BNK 규칙 위반
...
### 5-5. 빌드 산출물 실측 — 외부 URL 31건, `file://` 에셋 경로 깨짐
...
원본 확인 건은 해소됐다(§0). 남은 것은 순서와 M1 범위다.

1. **작업 순서를 §3 말미 제안대로 조정할지 결정** — 리브랜딩(M1) 직후 화면 10·11·12·13·6이 사실상 동시에 완성되므로, **M4(P2)를 M2보다 먼저** 털고 남는 시간을 화면 4(Graph RAG)에 투입하는 안. 가이드 §3의 순서(P0 → P1 → P2)를 그대로 갈지 여부.
2. **M1에 4건을 선행 포함** — 넷 다 나중에 하면 이미 만든 화면을 전부 재검증해야 한다:
   - HashRouter 교체 + `base: './'` 또는 singlefile 적용 (§5-1, §5-5 ①)
   - 폰트 정책 확정 (§5-3) — 레이아웃 검증의 전제
   - `localStorage` 3개소 → 메모리 스토어 (§5-4)
   - mock 디렉터리 규칙 정리 (§5-6) — `src/data/` 유지 + CLAUDE.md 수정을 권함

### M1 완료 판정 기준 (측정 가능하게)

- `grep -rniE 'kb|국민|kookmin|kbfg' src index.html tailwind.config.js package.json` → **0건**
- `npm run typecheck` → 통과 (Tenant 7→11 확장 시 연쇄 타입 에러 확인용)
- `npm run build` 후 `dist/` 외부 URL → **`reactjs.org` 1건 외 0건** (§5-5 ②)
- 빌드 산출물을 **네트워크 차단 상태에서 `file://` 로 열어** 라우팅 이동 확인 (§5-1)

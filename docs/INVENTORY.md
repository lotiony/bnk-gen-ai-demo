...
### 5-3. ~~웹폰트 — 로드는 없으나 렌더가 환경에 좌우된다~~ — ✅ 해소 (Pretendard Variable 번들 인라인)

> **해소 결과(2026-08-25)**: `pretendard` npm 패키지의 Variable woff2(2.0 MB)를 `assetsInlineLimit` 상향으로 base64 data URI 인라인. 앱이 쓰는 5개 weight(500·600·700·800·900)를 파일 하나로 커버한다. 단일 HTML 980 KB → **3.6 MB**. 외부 폰트 요청 0건, `file://` 1280×720 렌더 실측 확인.
>
> 아래는 해소 전 기록이다.
...
~~폰트 정책~~ · ~~작업 순서~~ — 둘 다 확정됐다(2026-08-25).
- 폰트: **Pretendard Variable 번들 인라인** (§5-3 해소)
- 순서: **가이드 원안 유지** — M2(P0 3종) → M3(P1) → M4(P2)

**다음: M2-1 화면 4 Graph RAG.** 자산 0건의 신규 개발이며, 선행 병목은 가상 규정 데이터셋(규정 12건 + 참조·개정 전후·전결권 관계) 설계다.
...
| `file://` 1280×720 렌더 (헤드리스 스크린샷) | 로그인 화면 정상 · Pretendard 적용 확인 |
| 미검증 | 전 화면 클릭 순회 · Edge · 프로젝터 실물 |
